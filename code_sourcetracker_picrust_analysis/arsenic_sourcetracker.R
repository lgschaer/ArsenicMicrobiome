...
ps <- readRDS("/home/lgschaer/old/MWarke_temp/unrarefied_T7soilR_phyloseq_07192023")
ps
...
ps2 <- ps %>%
  ...
  subset_samples(sample_sums(ps_filt) > 1000)
...
#write table
write.table(t_count_table, "/home/lgschaer/old/MWarke_temp/t_count_table.txt", sep="\t", row.names=TRUE, col.names=NA, quote=FALSE)

##next step: use putty to convert to .biom table

####### Processing Sourcetracker Output

sdataOG <- as.csv("/home/lgschaer/old/MWarke_temp/samples_metadata_soilarsenic_miceobiome.csv", row.names = 1, header = TRUE, sep = ",", check.names = TRUE, stringsAsFactors = TRUE) %>%
...
mxProp <- read.table("/home/lgschaer/old/MWarke_temp/sourcetracker_out2/mixing_proportions.txt") %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column(var = "SampleID") %>%
  pivot_longer(cols = c("PEroot", "PEshoot", "PEsoilB", "PEsoilR", "PVroot", "PVshoot", "PVsoilB",
                        "PVsoilR",  "soilB", "Unknown"), names_to = "Source", values_to = "Proportion") %>%
...
mxProp2 <- mxProp %>%
  left_join(sdataOG, by = c("SampleID")) %>%
  filter(Sample.type != "shoot") %>%
  filter(!is.na(Fern.type)) %>%
  #filter(Timepoint == "T7") %>%
  group_by(Arsenic.concentration, Fern.type, Sample.type, Source) %>%
  summarise(
...
#any(is.na(mxProp2$Fern.type))
head(mxProp2)


colors <- c("darkcyan", "lightblue", "firebrick", "cyan", "olivedrab4", "chocolate1", "purple4", "lightgoldenrod", "orange", "grey")

ggplot(mxProp2, aes(x=Fern.type, y = meanProp))+
  geom_col(mapping = aes(fill = Source), color = "black", position = "fill", show.legend = TRUE)+
  facet_nested(rows = vars(Arsenic.concentration), cols = vars(Sample.type), space = "free", scales = "free")+
